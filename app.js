...
// ══════════════════════════════════════════════
Luar biasa! UratKu sudah tampil sangat bagus! Saya lihat:

Grafik tren asam urat berjalan ✅
Riwayat serangan muncul ✅
Pesan dari dokter muncul ✅
Hidrasi tracker berjalan ✅
Card AI UratKu sudah muncul di bawah ✅

Tapi ada error "Gagal memuat analisis AI" — ini karena Claude API tidak bisa dipanggil langsung dari browser (masalah keamanan CORS).
Solusinya: kita buat versi AI yang lebih sederhana tapi tetap cerdas dan personal — analisis berbasis logika klinis menggunakan data pasien, tanpa perlu memanggil API eksternal.
Buka app.js → cari fungsi loadAIAnalysis → ganti SELURUHNYA dengan ini:
javascriptasync function loadAIAnalysis() {
  ...
  const seranganBulanIni = serangan.filter(s => {
    const tgl = new Date(s.tanggal_serangan);
    const sekarang = new Date();
    return (sekarang - tgl) < 30 * 24 * 60 * 60 * 1000;
  }).length;
  ...
  // Analisis makanan pemicu
  const semuaMakanan = serangan.map(s => s.makanan_pemicu).filter(Boolean).join(' ').toLowerCase();
  const pemicuMap = {
    'seafood/laut': ['seafood','udang','kepiting','cumi','kerang','ikan asin','teri'],
    'jeroan': ['jeroan','hati','ampela','otak','ginjal'],
    'emping': ['emping','melinjo'],
    'alkohol': ['alkohol','bir','tuak','arak'],
    'daging merah': ['daging','sapi','kambing','babi'],
    'kacang': ['kacang'],
  };
  const pemicuDitemukan = [];
  for (const [label, keywords] of Object.entries(pemicuMap)) {
    if (keywords.some(k => semuaMakanan.includes(k))) pemicuDitemukan.push(label);
  }
  ...
  if (serangan.length === 0) {
    ...
    polaTeks = `Dalam ${serangan.length} serangan terakhir, sendi yang paling sering terkena adalah ${sendiTerbanyak} dengan rata-rata skala nyeri ${nyeriRata.toFixed(1)}/10. Tren asam urat Anda ${auTren === 'naik' ? 'sedang meningkat — perlu perhatian lebih' : auTren === 'turun' ? 'sedang membaik — pertahankan!' : 'relatif stabil'}.`;
  }

  // Rekomendasi diet personal
  const rekomendasiDiet = [];
  if (auTerakhir >= 7) {
    rekomendasiDiet.push('Kurangi konsumsi seafood, jeroan, dan emping melinjo secara signifikan');
    rekomendasiDiet.push('Minum minimal 2.5 liter air putih per hari untuk membantu ekskresi asam urat');
  }
  if (pemicuDitemukan.includes('seafood/laut')) rekomendasiDiet.push('Hindari seafood terutama udang, kepiting, dan ikan asin — terbukti memicu serangan Anda');
  if (pemicuDitemukan.includes('jeroan')) rekomendasiDiet.push('Hentikan konsumsi jeroan (hati, ampela, otak) — sangat tinggi purin');
  if (pemicuDitemukan.includes('alkohol')) rekomendasiDiet.push('Hindari alkohol — menghambat ekskresi asam urat oleh ginjal');
  if (rekomendasiDiet.length < 3) {
    rekomendasiDiet.push('Perbanyak konsumsi buah ceri, stroberi, dan jeruk — mengandung vitamin C yang menurunkan asam urat');
    rekomendasiDiet.push('Konsumsi susu rendah lemak dan yogurt setiap hari — terbukti menurunkan risiko serangan gout');
    rekomendasiDiet.push('Batasi minuman manis dan jus kemasan — fruktosa meningkatkan produksi asam urat');
  }

  // Pesan motivasi
  const motivasi = auTren === 'turun' && auTerakhir < 6
    ? 'Asam urat Anda terkontrol dengan baik — teruskan gaya hidup sehat ini!'
    : auTerakhir >= 9
    ? 'Kadar asam urat perlu segera diturunkan — pastikan tidak melewatkan obat dan segera hubungi dokter Anda.'
    : 'Dengan disiplin minum obat dan menjaga pola makan, asam urat Anda pasti bisa terkontrol!';
  ...
  el.innerHTML = `
    <div style="display:flex;flex-direction:column;gap:10px">
      <div style="background:#F5F3FF;border:1px solid #DDD6FE;border-left:4px solid #7C3AED;border-radius:8px;padding:12px">
        ...
        ${pemicuDitemukan.length > 0 ? `<div style="margin-top:6px;font-size:11px;color:#6D28D9"><strong>Makanan pemicu teridentifikasi:</strong> ${pemicuDitemukan.join(', ')}</div>` : ''}
      </div>
      ...
      <div style="background:${risikoBg};border:1px solid ${risikoColor}40;border-radius:8px;padding:12px">
        ...
        <div style="font-size:12px;color:#374151">
          AU terakhir ${auTerakhir} mg/dL (rata-rata ${auRata.toFixed(1)} mg/dL), tren ${auTren}.
          ${seranganBulanIni > 0 ? `${seranganBulanIni} serangan dalam 30 hari terakhir.` : 'Tidak ada serangan dalam 30 hari terakhir.'}
        </div>
      </div>
      ...
      <div style="background:#F0FDF4;border:1px solid #BBF7D0;border-left:4px solid #16A34A;border-radius:8px;padding:12px">
        ...
        ${rekomendasiDiet.slice(0,4).map(r => `
          <div style="display:flex;gap:6px;margin-bottom:5px;font-size:12px;color:#374151;align-items:flex-start">
